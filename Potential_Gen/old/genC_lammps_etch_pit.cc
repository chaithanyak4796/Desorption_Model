...
int etch_pit(double *center, double *axis_len, double x, double y, double z)
{
  double val = 0.0;
  val = pow((x-center[0])/axis_len[0],2) + pow((y-center[1])/axis_len[1],2) + pow((z-center[2])/axis_len[2],2);
...
int main(int argc, char**argv)
{
    if (argc != 3)
    {
      cout << "The diamter and height of the etch pit must be given as input" << endl;
      exit (EXIT_FAILURE);
    }

    double D,H;
    double *center, *axis_len;
    D = atof(argv[1]);
    H = atof(argv[2]);

    axis_len = new double [3];
    center   = new double [3];
    ...
    // Determine the center of the pit
    center[0] = 0.5 * ( box[1]-box[0]  );
    center[1] = 0.5 * ( box[3]-box[2]  );
    center[2] = 0 * ( nz-0.5 ) * lattice_c;

    cout<<"The center of the etch pit is located at  x : "<<center[0]<<", y : "<<center[1]<<", z : "<<center[2]<<endl<<endl;
    

    // output some statistics to the command line
    cout << "Initial # of atoms :\t" << n_atoms << endl;
       ...
       for (int j = 0; j < ny; ++j)
       {
         for (int k = 0; k < nz; k++)
	  	 {  	 	
			...
			check = etch_pit(center,axis_len,pos[0],pos[1],pos[2]);
			if(check >= 1.0)
			  {
   ...
   cout << "Final # of atoms:\t" << particle_data.size() << endl;
    ...
    */
    ...
    output_file.open("graphite_etch_pit.lmp");
    ...
    output_file << "\t\t\t\t " << particle_data.size() << " atoms" << endl;
